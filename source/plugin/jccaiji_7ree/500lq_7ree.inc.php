...
foreach ($result_7ree as $key => $value){
	...
	*/
	if($key>0){
			...
			$change_7ree['peilv_7ree'][0] = floatval($value[9]);
			$change_7ree['peilv_7ree'][1] = floatval($value[10]);
...
function get_date_7ree($time_7ree){
		global $_G;
		$return_7ree = '';
		$return_7ree = strtotime(gmdate("Y-", $_G['timestamp'] + $_G['setting']['timeoffset'] * 3600).str_insert($time_7ree, 5, ' '));
		return $return_7ree;
}
...
function get_peilv_7ree($peilv_7ree){//足球让球赔率
		...
		if($peilv_7ree){
			$return_7ree[0]=substr($peilv_7ree,0,4);
			$return_7ree[1]=substr($peilv_7ree,4,4);
			$return_7ree[2]=substr($peilv_7ree,8,4);
		}
		return $return_7ree;
}
