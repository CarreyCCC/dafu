...
/*
	ID: jccaiji_7ree
	[www.7ree.com] (C)2007-2014 7ree.com.
	This is NOT a freeware, use is subject to license terms
	Update: 2015/5/18 11:31
...
//////////////////////////足球赛事
$ch_7ree = curl_init();
//$url1_7ree = "https://caipiao.taobao.com/lottery/order/lottery_jczq_hhgg_by_client.htm"; 
$url1_7ree = "http://trade.500.com/jczq/"; 
echo($url1_7ree);
curl_setopt ($ch_7ree,  CURLOPT_URL, $url1_7ree);
curl_setopt($ch_7ree, CURLOPT_HEADER, 1);
...
if($currentlang=="SC_GBK"){
	$info1_7ree = iconv("utf-8", "gb2312//IGNORE",$info1_7ree);
}
*/


$pos_7ree = strpos($info1_7ree,'<div class="bet_th" id="bet_th"');
$info1_7ree = substr($info1_7ree,$pos_7ree);

$pos2_7ree = strpos($info1_7ree,'<div class="buy_bar_area l_buy_area"');
$info1_7ree = substr($info1_7ree,0,$pos2_7ree);


$info1_7ree = str_replace('<a href="javascript:toHot(', '', $info1_7ree);
...
//$pos_7ree = strpos($info1_7ree,"<table>");
echo("<pre>");
print_r($result_7ree);
//print_r($blacklist_7ree);
//print_r($whitelist_7ree);	
//showmessage("pos=".$pos_7ree);
	
foreach ($result_7ree as $key => $value){
	/*
	if($value[0]==lang('plugin/jccaiji_7ree','php_lang_yincang_7ree')){
		$date_7ree = substr($value[1],0,10);
	}
	*/
	if($key>0){
			...
			if($cell0_len_7ree>3){
				$data_7ree=mb_substr($value[0], 3, 10, "$lang_7ree");
			}

			//$racetime_7ree = $data_7ree.' '.mb_substr($value[2], 0, 5, 'gbk');
			//$change_7ree['date_7ree'] =strtotime($racetime_7ree) ;
			$change_7ree['date_7ree'] =get_date_7ree($data_7ree,mb_substr($value[2], 0, 5, "$lang_7ree"));
			...
			$change_7ree['peilv_7ree'] = get_peilv_7ree($value[7]);
			if($vars_7ree['rangqiu_7ree']){
			        $change_7ree['rangqiu_7ree'] = get_rangqiu_7ree($value[6]);
			}else{
					$change_7ree['rangqiufang_7ree'] = 0;
					$change_7ree['rangqiuway_7ree'] = 0;
			}
			/*
			$change_7ree['zhusheng_7ree'] = $vars_7ree['peilv_7ree'] ?  floatval($peilv_7ree[1]) : $vars_7ree['peilv_zhu_7ree'];
			$change_7ree['ping_7ree'] = $vars_7ree['peilv_7ree'] ?  floatval($peilv_7ree[2]) : $vars_7ree['peilv_ping_7ree'];
			$change_7ree['zhufu_7ree'] = $vars_7ree['peilv_7ree'] ?  floatval($peilv_7ree[3]) : $vars_7ree['peilv_ke_7ree'];
			*/
			if($change_7ree['date_7ree']<=$_G['timestamp']+86400*3) $result2_7ree[] = $change_7ree;
			}
...
function get_date_7ree($date_7ree,$time_7ree){
		global $_G;
		$return_7ree = '';
		$clock_7ree = explode(':',$time_7ree);
		if($clock_7ree[0]<12){
			$day_jinwei_7ree=86400;
		}else{
			$day_jinwei_7ree=0;
		}
		$return_7ree = strtotime($date_7ree.' '.$time_7ree)+$day_jinwei_7ree;
		return $return_7ree;
}
...
function get_peilv_7ree($peilv_7ree){//足球让球赔率
		...
		if($peilv_7ree){
			$return_7ree[0]=substr($peilv_7ree,12,4);
			$return_7ree[1]=substr($peilv_7ree,16,4);
			$return_7ree[2]=substr($peilv_7ree,20,4);
		}
		return $return_7ree;
}
...
function get_rangqiu_7ree($rangqiu_7ree){//让球数分析
		$return_7ree = array();
		$rangqiu_7ree=intval(substr($rangqiu_7ree,1,2));
		if($rangqiu_7ree<0){
