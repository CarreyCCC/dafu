...
<form id="pmform_{$pmid}" name="pmform_{$pmid}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=pm&op=send&touid=$touid&pmid=$pmid&mobile=2" >
	...
	<div class="b_p">
		<div class="bz_post_msg_from">
			<ul>
				<!--{if !$touid}-->
				<li><input type="text" value="" tabindex="1" class="bzinp" autocomplete="off" id="username" name="username" placeholder="{lang addressee}"></li>
				<!--{/if}-->
				<li>
				...
				<li>
					...
					<button id="pmsubmit_btn" class="btn_pn_pm btn_pn_grey_pm" disable="true"><span>{lang sendpm}</span></button>
				</li>
			</ul>
		</div>
	</div>
